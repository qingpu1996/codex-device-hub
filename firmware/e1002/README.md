...
Codex 额度页的 5 分钟 timer wake 仍会联网检查最新数据，但只有套餐、状态、token 用量、额度百分比、重置文案等实际显示内容变化时才刷新电子纸。电池 ADC 波动、`generatedAt` 和未直接显示的原始 `resetsAt` 不会单独触发额度页刷新；电池文字会在额度变化、手动刷新或重新进入该页时随页面一起更新。

食谱和天气页的 hash 继续包含 PageId、页码、页面内容、电池显示值、食谱图片 hash 和天气显示内容。
