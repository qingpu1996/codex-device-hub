...
{
    // The deck initializes I2C before codec init; reuse that bus instead of
    // trying to install a second master on the same port.
    i2c_master_bus_handle_t bus_handle = NULL;
    if (i2c_master_get_bus_handle(port, &bus_handle) == ESP_OK && bus_handle) {
        return bus_handle;
    }
